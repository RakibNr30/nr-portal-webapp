                    ...
                    <div class="card card-gray-dark card-outline">
                        <div class="card-header">
                            ...
                            <a href="{{ route('backend.cms.article.index') }}" type="button"
                               class="btn btn-success btn-sm text-white float-right">View Article List</a>
                        </div>
                        {!! Form::open(['url' => route('backend.cms.article.store'), 'method' => 'article', 'files' => true]) !!}
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="article_category_id"
                                               class="@error('article_category_id') text-danger @enderror">Article
                                            Category</label>
                                        <select id="article_category_id" name="article_category_id"
                                            ...
                                            @foreach($articleCategories as $articleCategory)
                                                <option
                                                    value="{{ $articleCategory->id }}" {{ old('article_category_id') ? (old('article_category_id') == $articleCategory->id ? 'selected' : '') : '' }}>{{ $articleCategory->title }}</option>
                                            @endforeach
                                        </select>
